...
export default {
  ...
  methods: {
    ...
    onMouseDown(event){
      if(event.button == 1) { window.open(`./docs#${this.pdata.processData.process.identifier}`) }
      if(event.button != 0) { return }
      if(this.currentMenuData.isOpen){
        this.currentMenuData.isOpen = false;
        if(this.currentMenuData.processData != this.pdata){
          Vue.nextTick(() => {
            this.currentMenuData.processData = this.pdata;
    ...
    oncontextmenu(){
      if(this.currentMenuData.isOpen && this.currentMenuData.processData == this.pdata)
        this.currentMenuData.isOpen = false;
  ...
  >
    ...
    <g class="node-header" :style="{fill: \`\${pdata.processData.process.color}\`}" @mousedown="onMouseDown">
      <rect class="header-round-rect" width="200" height="40" x="2" y="2" rx="4" ry="4" />
      <rect class="header-rect" width="200" height="36" x="2" y="6" />
      <text class="header-title" x="102" y="30">{{pdata.processData.process.label}}</text>
    </g>
    <g class="node-content">
      ...
      <g class="inputs">
        <port v-for="(d, i) in pdata.processData.process.inPorts" :isInput="true" :data="d" :index="i" 
          :top="getPortHeight(i, pdata.processData.process.inPorts.length)" ref="inPorts" @startdragging="startDragging"></port>
      </g>
      <g class="outputs">
        <port v-for="(d, i) in pdata.processData.process.outPorts" :isInput="false" :data="d" :index="i" 
          :top="getPortHeight(i, pdata.processData.process.outPorts.length)" ref="outPorts" @startdragging="startDragging"></port>
      </g>
    </g>
  </g> 
  `
