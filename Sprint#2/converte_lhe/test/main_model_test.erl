...
main_exception5_test() -> 
    ?assertEqual({false, <<"failed convertion to store: d">>}, 
        main_model:main(["1", "2", "3"], "x1/$toInt/#1/:d/i3/$sum/#2")).

main_exception6_test() -> 
    ?assertEqual({false, <<"failed convertion to variable: h">>}, 
...
main_exception9_test() -> 
    ?assertEqual({false, <<"not enough values on the stack: [1]">>}, 
